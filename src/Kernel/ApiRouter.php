            ...
            if ($endpoint->getPath() === $urn) {
                if ($endpoint->getRequestMethod() === $_SERVER['REQUEST_METHOD']) 
                {
                    if ($endpoint->isProtected()) {
                        //on doit creer une classe qui gere la sécurité des endpoints
                        if (Guard::check()) {
                        //cette ligne de code fait en sorte de retrouver la bonne route
                        //assigne la variable $endpointFound(qui est null à la base)
